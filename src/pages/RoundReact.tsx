...
const RoundReact: React.FC = () => {
    ...
    const [status, setStatus] = useState<'selecting' | 'coding' | 'success'>('selecting');
    ...
    const question = selectedQ !== null ? REACT_QUESTIONS[selectedQ] : null;
    ...
    }, [question, setTimer, tickTimer, stopTimer, setPhase, setRoundInProgress, status]);
    ...
    // Low Time Alert
    useEffect(() => {
        if (timeLeft === 300 && timerActive && question) {
            setAlert({
                isOpen: true,
        ...
        if (timeLeft === 60 && timerActive && question) {
            setAlert({
                isOpen: true,
    ...
    const validateCode = (currentCode: string) => {
        ...
        validateTimeoutRef.current = setTimeout(() => {
            ...
            if (question.id === 1) {
                ...
                if (currentCode.includes('value={text}') && currentCode.includes('onChange={(e) => setText(e.target.value)}')) {
                    isFixed = true;
                }
            }
    ...
    const handleSuccess = () => {
        ...
        // Max 500 Points: 250 Base + Up to 250 for time
        // Base 600s (10 mins)
        const speedBonus = Math.floor((timeLeft / 600) * 250);
        ...
        setTimeout(() => {
            // nextReactQuestion(); // Removed for single question mode
            setPhase('dashboard'); // Back to dashboard after success
        }, 3000);
    };

    // if (!question) return <div>Loading...</div>; // Removed to allow selection screen

    const handleSelect = (idx: number) => {
        setSelectedQ(idx);
        // setCode(REACT_QUESTIONS[idx].buggy_code || ''); // Handled in useEffect
        setStatus('coding');
        setTimer(600); // 10 minutes per challenge
        setPreviewStatus('broken');
        setErrorMsg('Runtime Error: Component failed to mount properly.');
        ...
        <div className="w-full h-full max-w-[1600px] flex flex-col gap-4 p-4 z-10 relative">
            {status === 'selecting' && (
                <div className="flex flex-col items-center gap-8 pt-12">
                    <h2 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300 font-display drop-shadow-[0_0_15px_rgba(0,100,255,0.5)]">
                        ROUND 2: REACT PROTOCOL - SELECT MODULE
                    </h2>
                    <div className="flex flex-wrap justify-center gap-6 max-w-5xl">
                        {REACT_QUESTIONS.map((q, idx) => (
                            <motion.div
                                key={q.id}
                                whileHover={{ scale: 1.05, borderColor: '#3b82f6' }}
                                onClick={() => handleSelect(idx)}
                                className="w-72 h-auto min-h-[18rem] bg-black/60 backdrop-blur-md border border-blue-500/30 rounded-2xl p-6 cursor-pointer hover:bg-blue-900/10 transition-all flex flex-col justify-between shadow-[0_0_30px_rgba(0,0,0,0.3)] group"
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <span className="text-blue-400 font-bold font-mono text-xs px-2 py-1 bg-blue-900/30 rounded border border-blue-500/30">
                                        {q.difficulty}
                                    </span>
                                    <Code size={20} className="text-gray-500 group-hover:text-blue-400 transition-colors" />
                                </div>
                                <div className="mt-auto">
                                    <h3 className="text-2xl font-bold text-white mb-2 font-heading group-hover:text-blue-300 transition-colors">{q.title}</h3>
                                    <p className="text-gray-400 text-xs mt-2 line-clamp-3 leading-relaxed font-mono">{q.description}</p>
                                </div>
                            </motion.div>
                        ))}
                    </div>
                    <button
                        ...
                        className="mt-8 px-8 py-3 bg-red-900/20 border border-red-500/30 text-red-400 font-mono text-sm hover:bg-red-900/40 hover:text-red-300 transition-all rounded-lg"
                    >
                ...
                <>
                    ...
                    <div className="flex justify-between items-center bg-gradient-to-r from-black/80 via-blue-900/20 to-black/80 p-6 border border-blue-500/30 rounded-xl backdrop-blur-md shadow-[0_0_30px_rgba(0,100,255,0.15)] relative overflow-hidden">
                        ...
                        <div className="relative z-10 flex items-center gap-6 flex-1">
                            <button onClick={() => setStatus('selecting')} className="text-blue-500/70 hover:text-blue-400 transition-colors mr-2">
                                &larr; BACK
                            </button>
                            ...
                            <div className="flex-1">
                                ...
                                {/* Solved Count & Qualification Status - REMOVED for Single Selection Mode */}
                            </div>
                        </div>
                        ...
                        <div className="relative z-10 flex gap-4">
                            ...
                            <div className="bg-black/60 px-6 py-3 rounded-xl border border-yellow-500/30 backdrop-blur">
                                ...
                                <div className="text-2xl font-bold text-yellow-400 font-mono">
                                    {250 + Math.floor((timeLeft / 600) * 250)} <span className="text-sm text-gray-500">PTS</span>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                {previewStatus === 'fixed' && !isReloading && (
                                    ...
                                    >
                                        <div className="text-center text-white">
                                            ...
                                            >
                                                +{250 + Math.floor((timeLeft / 300) * 250)} POINTS
                                            </motion.p>
                                            <motion.div
                                                initial={{ opacity: 0 }}
                                                animate={{ opacity: 1 }}
                                ...
                                <div className="flex-1 p-8 flex flex-col items-center justify-center">
                                    ...
                                    {question.id === 3 && (
                                        <motion.div
                                            ...
                                            className="text-center p-8 border rounded-xl shadow-sm bg-gray-50 max-w-sm w-full"
                                        >
                                            <h3 className="text-xl font-bold text-gray-800 mb-6">Live Input</h3>
                                            <div className="text-2xl text-gray-800 mb-8 font-mono min-h-[40px] border-b-2 border-gray-200">
                                                {previewStatus === 'fixed' ? 'Hello World' : '...'}
                                            </div>
                                            <input
                                                disabled
